...
namespace Frapid.NPoco
{
    ...
    public class Relator
    {
        ...
        {
            return this.OneToMany(main, sub, idFunc, null);
        }
        ...
        {
            if (main == null)
                return (T)this.onetomanycurrent;

            if (this.property1 == null)
            {
                this.property1 = typeof(T).GetProperties().Where(x => typeof(ICollection<TSub>).IsAssignableFrom(x.PropertyType)).FirstOrDefault();
                if (this.property1 == null)
                    ThrowPropertyNotFoundException<T, ICollection<TSub>>();
            }

            if (this.onetomanycurrent != null && idFunc((T)this.onetomanycurrent).Equals(idFunc(main)))
            {
                ((ICollection<TSub>)this.property1.GetValue((T)this.onetomanycurrent, null)).Add(sub);
                return default(T);
            }

            T prev = (T)this.onetomanycurrent;
            this.onetomanycurrent = main; 

            bool nullMany = sub == null || (subIdFunc != null && subIdFunc(sub).Equals(GetDefault(subIdFunc(sub).GetType())));
            this.property1.SetValue((T) this.onetomanycurrent, nullMany ? new List<TSub>() : new List<TSub> {sub}, null);
