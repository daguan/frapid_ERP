namespace Frapid.NPoco.Expressions
{
    public class FirebirdSqlExpression<T> : SqlExpression<T>
    {
        ...
        {
            if (length >= 0)
                return string.Format("substring({0} FROM {1} FOR {2})", columnName, this.CreateParam(startIndex), this.CreateParam(length));
            else
                return string.Format("substring({0} FROM {1})", columnName, this.CreateParam(startIndex));
        }
    }
}