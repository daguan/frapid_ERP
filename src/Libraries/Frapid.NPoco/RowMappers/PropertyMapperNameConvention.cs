...
namespace Frapid.NPoco.RowMappers
{
    public static class PropertyMapperNameConvention
    {
        ...
        {
            foreach (PocoMember pocoMember in pocoMembers.OrderBy(x => x.PocoMemberChildren.Count != 0))
            {
                ...
                levelMonitor.Level++;
                foreach (PocoMemberLevel pocoMemberLevel in FlattenPocoMembers(pocoMember.PocoMemberChildren, levelMonitor))
                {
