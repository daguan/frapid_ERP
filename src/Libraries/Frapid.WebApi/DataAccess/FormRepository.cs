...
namespace Frapid.WebApi.DataAccess
{
    public class FormRepository: DbAccess, IFormRepository
    {
        ...
        public async Task<dynamic> GetAsync(object primaryKey)
        {
            ...
            return (await Factory.GetAsync<dynamic>(this.Database, sql, primaryKey)).FirstOrDefault();
        }
        ...
        public async Task<dynamic> GetFirstAsync()
        {
            ...
            return (await Factory.GetAsync<dynamic>(this.Database, sql)).FirstOrDefault();
        }
        ...
        public async Task<IEnumerable<CustomField>> GetCustomFieldsAsync(string resourceId)
        {
            ...
            if(string.IsNullOrWhiteSpace(resourceId))
            {
                ...
                return await Factory.GetAsync<CustomField>(this.Database, sql);
            }
            ...
            return await Factory.GetAsync<CustomField>(this.Database, sql, this.FullyQualifiedObjectName, resourceId);
        }
        ...
        public async Task<object> AddOrEditAsync(Dictionary<string, object> item, List<CustomField> customFields)
        {
            ...
            {
                await this.UpdateAsync(item, primaryKeyValue, customFields);
            }
            else
            {
                primaryKeyValue = await this.AddAsync(item, customFields, true);
            }
                ...
                using(var db = DbProvider.GetDatabase(this.Database))
                {
                    using(var transaction = db.GetTransaction())
                    {
                        ...
                        foreach(var item in items)
                        {
                            ...
                            {
                                string columns = string.Join(",", item.Where(x => !x.Key.Equals(this.PrimaryKey)).Select(x => Sanitizer.SanitizeIdentifierName(x.Key)));
                                ...
                                result.Add(await db.ExecuteScalarAsync<object>(sql));
                            }
                        }
        ...
        public async Task UpdateAsync(Dictionary<string, object> item, object primaryKeyValue, List<CustomField> customFields)
        {
            ...
            using(var db = DbProvider.GetDatabase(this.Database))
            {
                ...
                await db.ExecuteAsync(sql);
                await this.AddCustomFieldsAsync(primaryKeyValue, customFields);
            }
        }
        ...
        public async Task DeleteAsync(object primaryKey)
        {
            ...
            await Factory.NonQueryAsync(this.Database, sql, primaryKey);
        }
        ...
        public async Task<IEnumerable<dynamic>> GetPaginatedResultAsync(long pageNumber)
        {
            ...
            return await Factory.GetAsync<dynamic>(this.Database, sql, offset);
        }
        ...
        public async Task<IEnumerable<Filter>> GetFiltersAsync(string tenant, string filterName)
        {
            using(var db = DbProvider.GetDatabase(this.Database))
            {
                return await db.Query<Filter>().Where(u => u.ObjectName.Equals(this.FullyQualifiedObjectName) && u.FilterName.ToLower().Equals(filterName.ToLower())).ToListAsync();
            }
        }
        ...
        public async Task<object> AddAsync(Dictionary<string, object> item, List<CustomField> customFields, bool skipPrimaryKey)
        {
            ...
            using(var db = DbProvider.GetDatabase(this.Database))
            {
                string columns = string.Join
                    (",", skipPrimaryKey ? item.Where(x => !x.Key.Equals(this.PrimaryKey)).Select(x => Sanitizer.SanitizeIdentifierName(x.Key)) : item.Select(x => Sanitizer.SanitizeIdentifierName(x.Key)));

                string parameters = string.Join(",", Enumerable.Range(0, skipPrimaryKey ? item.Count - 1 : item.Count).Select(x => "@" + x));

                var arguments = skipPrimaryKey ? item.Where(x => !x.Key.Equals(this.PrimaryKey)).Select(x => x.Value).ToArray() : item.Select(x => x.Value).ToArray();
                ...
                var primaryKeyValue = await db.ExecuteScalarAsync<object>(sql);
                await this.AddCustomFieldsAsync(primaryKeyValue, customFields);
                return primaryKeyValue;
            }
        }
        ...
        {
            string sql = $"DELETE FROM config.custom_fields WHERE custom_field_setup_id IN(" + "SELECT custom_field_setup_id " + "FROM config.custom_field_setup " +
                         "WHERE form_name=config.get_custom_field_form_name('{this.FullyQualifiedObjectName}')" + ");";

            await Factory.NonQueryAsync(this.Database, sql);
            ...
            {
                sql = $"INSERT INTO config.custom_fields(custom_field_setup_id, resource_id, value) " + "SELECT config.get_custom_field_setup_id_by_table_name('{this.FullyQualifiedObjectName}', @0), " + "@1, @2;";
        ...
        public async Task<EntityView> GetMetaAsync()
        {
            ...
            return await EntityView.GetAsync(this.Database, this.PrimaryKey, this._ObjectNamespace, this.GetTableName());
        }
    }
}