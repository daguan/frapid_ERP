    ...
    public sealed class FrapidOutputCache: DonutOutputCacheAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if(this.CacheSettings.IsServerCachingEnabled)
            {
                ...
                if((this.CacheSettings.Options & OutputCacheOptions.NoCacheLookupForPosts) != OutputCacheOptions.NoCacheLookupForPosts ||
            ...
            filterContext.HttpContext.Items[cacheKey] = new Action<bool>
                ...
                hasErrors =>
                {
                    this.Callback(filterContext, hasErrors, cacheKey, originalWriter, cachingWriter);
                });
        }

        private void Callback(ActionExecutingContext filterContext, bool hasErrors, string cacheKey, TextWriter originalWriter, StringWriter cachingWriter)
        {
            ...
            filterContext.HttpContext.Response.Write(this.DonutHoleFiller.RemoveDonutHoleWrappers(cacheItem.Content, filterContext, this.CacheSettings.Options));

            if(this.CacheSettings.IsServerCachingEnabled &&
               filterContext.HttpContext.Response.StatusCode == 200)
            {
                this.OutputCacheManager.AddItem(cacheKey, cacheItem, DateTime.UtcNow.AddSeconds(this.CacheSettings.Duration));
            }
        }
    }
}