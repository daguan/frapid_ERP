...
namespace Frapid.Config.DataAccess
{
    ...
    public class CustomField : DbAccess, ICustomFieldRepository
    {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object AddOrEdit(dynamic customField, List<Frapid.DataAccess.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
        ...
        /// <returns>Returns collection of "CustomField" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public IEnumerable<Frapid.Config.Entities.CustomField> GetWhere(long pageNumber, List<Frapid.DataAccess.Filter> filters)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
