...
namespace Frapid.Config.DataAccess
{
    ...
    public class FlagType : DbAccess, IFlagTypeRepository
    {
        ...
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public object AddOrEdit(dynamic flagType, List<Frapid.DataAccess.CustomField> customFields)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
        ...
        /// <returns>Returns collection of "FlagType" class.</returns>
        /// <exception cref="UnauthorizedException">Thown when the application user does not have sufficient privilege to perform this action.</exception>
        public IEnumerable<Frapid.Config.Entities.FlagType> GetWhere(long pageNumber, List<Frapid.DataAccess.Filter> filters)
        {
            if (string.IsNullOrWhiteSpace(this._Catalog))
            {
                return null;
