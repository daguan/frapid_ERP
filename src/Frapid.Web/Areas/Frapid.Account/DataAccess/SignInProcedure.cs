...
{
    /// <summary>
    /// Prepares, validates, and executes the function "account.sign_in(_email text, _office_id integer, _browser text, _ip_address text, _culture text)" on the database.
    /// </summary>
    public class SignInProcedure : DbAccess, ISignInRepository
    {
        ...
        public SignInProcedure(string email, int officeId, string browser, string ipAddress, string culture)
        {
            this.Email = email;
        ...
        public IEnumerable<DbSignInResult> Execute()
        {
            ...
            string query = "SELECT * FROM account.sign_in(@Email, @OfficeId, @Browser, @IpAddress, @Culture);";

            query = query.ReplaceWholeWord("@Email", "@0::text");
            query = query.ReplaceWholeWord("@OfficeId", "@1::integer");
            query = query.ReplaceWholeWord("@Browser", "@2::text");
            query = query.ReplaceWholeWord("@IpAddress", "@3::text");
            query = query.ReplaceWholeWord("@Culture", "@4::text");
